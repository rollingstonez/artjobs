# -*- coding: utf-8 -*-
"""
한국문화예술교육진흥원(ARTE) 교육연수 크롤러.

수집 대상: 교원·문화예술교육사 등 대상 연수·교육과정 모집 공고.
게시판 위치: arte.or.kr > 교육연수 > 연수공지 — 실측 후 LIST_URL 확정.
...
실행: 저장소 최상위에서  python scripts/crawler/crawl_arte_edu.py [--dry-run]
"""
import re
import sys
...
SOURCE_CODE = "arte_edu"
SOURCE_NAME = "한국문화예술교육진흥원 교육연수"
REGION = "전국·온라인"
# ⚠️ 아래 URL 은 추정값. 브라우저에서 실제 목록 페이지를 열어 확인 후 수정.
LIST_URL = "https://www.arte.or.kr/training/notice/"
DETAIL_BASE = "https://www.arte.or.kr"
PAGE_PARAM = "page"
MAX_PAGES = 20
...
def parse_list(html):
    ...
    for tr in soup.select("table.board_list tbody tr, table tbody tr, ul.board_list li"):
        link = tr.select_one("a[href]")
        if not link:
            continue
        href = link.get("href", "")
        m = re.search(r"[?&](?:idx|no|seq|id)=(\d+)|/(\d+)(?:/|$)", href)
        if not m:
            continue
        ...
        # 날짜 칸에서 접수 기간 추출
        cells = [td.get_text(" ", strip=True) for td in tr.select("td, li")]
...
def fetch_detail(key):
    ...
    # 교육 기간(work_start/work_end)
    m_work = re.search(
        r"(?:교육|연수)\s*기간\s*[:：]?\s*(\d{4}[.\-/]\d{1,2}[.\-/]\d{1,2})\s*[~–]\s*(\d{4}[.\-/]\d{1,2}[.\-/]\d{1,2})",
        text,
    )
    if m_work:
