...
판정 묶음(위에서부터 우선 적용):
  ...
  C 목록 주소만 채우면 됨   robots 허용인데 목록 주소를 아직 실측하지 못함
  ...
  G 접속 실패 → 재확인        robots.txt 응답 없음(연결 실패). 며칠 뒤 다시
"""
import io
import json
import os
...
BUCKET_TITLE = {
    ...
    "C": "C. robots 는 허용 — 목록 주소만 브라우저로 확인해 채우면 됨",
...
BUCKET_ACTION = {
    ...
    "G": "재판정. 계속 실패하면 사이트 주소 자체를 다시 확인",
}
...
def main(json_path, out_path):
    ...
    # 공공데이터 요청 목록
    ask = [(s, r) for k in ("D", "E", "F", "G") for s, r in buckets[k] if is_public(s)]
    out.write("## 공공데이터 요청 목록 (E·F·D·G 중 공공기관)\n\n")
