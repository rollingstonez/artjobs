...
> 템플릿은 **커스텀 SMTP 를 연결한 뒤에만** 고칠 수 있다(무료 기본 발송은 템플릿 수정 불가, 시간당 2~4통 제한).
> Authentication → Emails → "Set up SMTP" → Resend(무료 3,000통/월) 등을 연결하고 나서 아래를 적용한다. 순서는 4번 참고.
> SMTP 를 아직 안 붙였으면 기본 템플릿으로도 동작은 한다. 단, 메일 링크를 **비밀번호 찾기를 누른 그 브라우저**에서 열어야 한다.
