...
export default async function AdminSourcesPage({ searchParams }: PageProps<"/admin/sources">) {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-stone-500">
        <span className="rounded-full bg-violet-50 px-1.5 py-0.5 text-[11px] font-semibold text-violet-700">아트누리 중복</span> 표시가 붙은 지역 문화재단은 공모·지원사업이 <b>아트누리(통합안내)</b>에 이미 다 모이므로 개별 수집 대상에서 빼 두었습니다(꺼진 채 보관). 그 재단의 직원 채용만 따로 필요할 때 켜세요.
      </p>
      {rows.length === 0 ? (
